...
def project_state_updates() -> dict[str, str]:
    ...
    return {
        "phase": "V3.9.20 + V4 mid-lock (T1–T14) + Part 1 bridge E2E",
        "focus": "V3.9.20 ticket↔memory linkage shipped; resume V4 T15 (#217)",
        "current_risk": (
            "Restart bus after code bumps; Actions on 127.0.0.1:8765 (not 8080); "
            "V4 final bump still at T24 (#226)."
        ),
        "next_action": (
            "Cursor claims #217 (T15 spark confidence decay). "
            "Midway QA: ticket.get include_memories, audit_memory_ticket_linkage."
        ),
        "what_changed": (
            "V3.9.20 (#264/#225): ticket lineage (sort=oldest, tickets.lineage, deep_sync history); "
            "memory_ticket_linkage (linked_ticket_ids_json, backfill, ticket.get include_memories); "
            "handoff Next Action false-close fix."
        ),
    }
