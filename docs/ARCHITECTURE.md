...
22. **V4.0 Cognitive Memory (in progress · mid-lock 2026-07-07)** — separate `sparks` / `spark_links` / `patterns` tables; `/api/cognitive/*` namespace; deterministic retrieval + graph + pattern safety + context orchestration (T1–T13). **Not complete** — depth modes, lifecycle, security, observability, release lock remain (T14–T24). See [V4.0_COGNITIVE_MEMORY_MID_LOCK.md](./V4.0_COGNITIVE_MEMORY_MID_LOCK.md).
