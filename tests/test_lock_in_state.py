...
class LockInStateTests(IsolatedDbTestCase):
    def test_project_state_updates_reference_quality_batch(self) -> None:
        ...
        self.assertIn("V3.9.12 shipped", updates["phase"])
        self.assertIn("V4 Spark Lanes", updates["focus"])
        self.assertIn("V3.9.12", updates["what_changed"])
        self.assertIn("portable", updates["what_changed"].lower())
    ...
    def test_refresh_project_state_writes_lock_in_fields(self) -> None:
        ...
        self.assertIn("V3.9.12 shipped", str(state["phase"]))
        self.assertIn("V4 Spark Lanes", str(state["focus"]))
