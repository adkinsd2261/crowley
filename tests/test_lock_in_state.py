...
class LockInStateTests(IsolatedDbTestCase):
    def test_project_state_updates_reference_quality_batch(self) -> None:
        ...
        self.assertIn("V3.9.13 shipped", updates["phase"])
        self.assertIn("V4 Spark Lanes", updates["focus"])
        self.assertIn("V3.9.13", updates["what_changed"])
        self.assertIn("actions", updates["what_changed"].lower())
    ...
    def test_refresh_project_state_writes_lock_in_fields(self) -> None:
        ...
        self.assertIn("V3.9.13 shipped", str(state["phase"]))
