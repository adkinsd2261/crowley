...
class LockInStateTests(IsolatedDbTestCase):
    def test_project_state_updates_reference_quality_batch(self) -> None:
        ...
        self.assertIn("V3.9.20", updates["phase"])
        self.assertIn("memory linkage", updates["focus"])
        self.assertIn("#217", updates["next_action"])
        self.assertIn("memory_ticket_linkage", updates["what_changed"])
    ...
    def test_refresh_project_state_writes_lock_in_fields(self) -> None:
        ...
        self.assertIn("V3.9.20", str(state["phase"]))
        self.assertIn("memory linkage", str(state["focus"]))
